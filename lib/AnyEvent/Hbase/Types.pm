...
sub write {
  ...
  if (defined $self->{sortedColumns}) {
    $xfer += $output->writeFieldBegin('sortedColumns', TType::LIST, 3);
    {
      $xfer += $output->writeListBegin(TType::STRUCT, scalar(@{$self->{sortedColumns}}));
      {
...
package Hbase::TScan;
use base qw(Class::Accessor);
Hbase::TScan->mk_accessors( qw( startRow stopRow timestamp columns caching filterString batchSize sortColumns ) );
